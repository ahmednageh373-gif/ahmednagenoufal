...
// Environment-aware API key getter
const getApiKey = (): string => {
    // Vite/local dev environments use import.meta.env with a VITE_ prefix.
    const localApiKey = (import.meta as any).env?.VITE_GEMINI_API_KEY;
    // The live platform injects process.env.API_KEY.
    const platformApiKey = process.env.API_KEY;

    const apiKey = localApiKey || platformApiKey;
    if (!apiKey) {
        throw new Error("An API Key must be provided. Please create a .env.local file with VITE_GEMINI_API_KEY=YOUR_KEY.");
    }
    return apiKey;
};
